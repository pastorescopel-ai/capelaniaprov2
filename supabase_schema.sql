...
-- 5. DIAGNÓSTICO E REPARO DE MATRÍCULAS (V6.2)
-- Objetivo: Converter IDs de UUID para BIGINT e limpar registros órfãos

/* 
  PASSO 1: Rodar no SQL Editor do Supabase para limpar registros com IDs corrompidos (UUIDs)
  
  -- Remove registros que não possuem ID numérico (resquícios de versões antigas)
  DELETE FROM pro_group_members WHERE id::text ~ '[a-zA-Z]';
  
  -- Garante que a sequência de IDs está correta
  SELECT setval(pg_get_serial_sequence('pro_group_members', 'id'), coalesce(max(id), 0) + 1) FROM pro_group_members;
*/

-- B. Reforço da Tabela de Matrículas
-- Garante que o ID seja BIGINT e autoincrementável se não fornecido
ALTER TABLE IF EXISTS pro_group_members 
  ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;

-- C. Indexação de Performance para buscas por Colaborador
