...
DROP FUNCTION IF EXISTS heal_sector_global(text, text);
...
BEGIN
    ...
    SELECT name INTO official_name FROM pro_sectors WHERE id = target_sector_id::bigint;
    ...
    -- Atualiza PGs
    UPDATE pro_groups
    SET sector_id = target_sector_id::bigint
    ...
    SET sector_id = target_sector_id::bigint, sector = official_name
...
-- 2. unify_identity_v6
DROP FUNCTION IF EXISTS unify_identity_v6(text, text, text);
...
BEGIN
    ...
    IF target_type = 'Colaborador' THEN
        ...
        FROM pro_staff WHERE id = target_id::bigint;
    ...
    WHERE orphan.class_id = official.class_id
      ...
      AND (
          (target_type = 'Colaborador' AND official.staff_id = target_id::bigint) OR
    ...
    IF target_type = 'Colaborador' THEN
        ...
        SET staff_id = target_id::bigint, student_name = official_name
    ...
    IF target_type = 'Colaborador' THEN
        UPDATE bible_study_sessions
        SET staff_id = target_id::bigint, name = official_name, participant_type = 'Colaborador', sector_id = COALESCE(sector_id, official_sector_id)
    ...
    IF target_type = 'Colaborador' THEN
        UPDATE staff_visits
        SET staff_id = target_id::bigint, staff_name = official_name, participant_type = 'Colaborador'
...
DROP FUNCTION IF EXISTS merge_identities_v6(text, text, text, text);
...
BEGIN
    ...
    IF target_type = 'Colaborador' THEN
        SELECT name, sector_id INTO official_name, official_sector_id FROM pro_staff WHERE id = target_id::bigint;
    ELSIF target_type = 'Paciente' THEN
        SELECT name INTO official_name FROM pro_patients WHERE id = target_id::bigint;
    ELSIF target_type = 'Prestador' THEN
        SELECT name INTO official_name FROM pro_providers WHERE id = target_id::bigint;
    ...
    IF source_type = 'Colaborador' THEN
        SELECT name INTO source_name FROM pro_staff WHERE id = source_id::bigint;
    ...
    IF source_type = 'Colaborador' AND target_type = 'Colaborador' THEN
        ...
        WHERE source_rec.class_id = target_rec.class_id
          AND source_rec.staff_id = source_id::bigint
          AND target_rec.staff_id = target_id::bigint
    ...
    IF source_type = 'Colaborador' THEN
        IF target_type = 'Colaborador' THEN
            UPDATE bible_class_attendees SET staff_id = target_id::bigint, student_name = official_name WHERE staff_id = source_id::bigint;
        ELSE
            UPDATE bible_class_attendees SET staff_id = NULL, student_name = official_name WHERE staff_id = source_id::bigint;
        END IF;
    ELSE
        IF target_type = 'Colaborador' THEN
            UPDATE bible_class_attendees SET staff_id = target_id::bigint, student_name = official_name WHERE student_name = source_name AND staff_id IS NULL;
        ELSE
    ...
    IF source_type = 'Colaborador' THEN
        IF target_type = 'Colaborador' THEN
            UPDATE bible_study_sessions SET staff_id = target_id::bigint, name = official_name, sector_id = COALESCE(sector_id, official_sector_id) WHERE staff_id = source_id::bigint;
        ELSE
            UPDATE bible_study_sessions SET staff_id = NULL, name = official_name, participant_type = target_type WHERE staff_id = source_id::bigint;
        END IF;
    ELSE
        IF target_type = 'Colaborador' THEN
            UPDATE bible_study_sessions SET staff_id = target_id::bigint, name = official_name, participant_type = 'Colaborador', sector_id = COALESCE(sector_id, official_sector_id) WHERE name = source_name AND staff_id IS NULL;
        ELSE
    ...
    IF source_type = 'Colaborador' THEN
        IF target_type = 'Colaborador' THEN
            UPDATE staff_visits SET staff_id = target_id::bigint, staff_name = official_name WHERE staff_id = source_id::bigint;
        ELSE
            UPDATE staff_visits SET staff_id = NULL, staff_name = official_name, participant_type = target_type WHERE staff_id = source_id::bigint;
        END IF;
    ELSE
        IF target_type = 'Colaborador' THEN
            UPDATE staff_visits SET staff_id = target_id::bigint, staff_name = official_name, participant_type = 'Colaborador' WHERE staff_name = source_name AND staff_id IS NULL;
        ELSE
    ...
    -- 7. VINCULAÇÃO EM PGs
    IF source_type = 'Colaborador' AND target_type = 'Colaborador' THEN
        UPDATE pro_group_members SET staff_id = target_id::bigint WHERE staff_id = source_id::bigint;
        UPDATE pro_history_records SET staff_id = target_id::bigint, staff_name = official_name WHERE staff_id = source_id::bigint;
    END IF;

    -- 8. DELETAR ORIGEM
    IF source_type = 'Colaborador' THEN DELETE FROM pro_staff WHERE id = source_id::bigint;
...
DROP FUNCTION IF EXISTS unify_and_deduplicate_identity(text, text);
...
BEGIN
    ...
    SELECT name, sector_id INTO official_name, official_sector_id 
    FROM pro_staff WHERE id = target_staff_id::bigint;
    ...
    WHERE orphan.class_id = official.class_id
      AND lower(regexp_replace(orphan.student_name, '[^a-zA-Z0-9]', '', 'g')) = clean_orphan
      AND official.staff_id = target_staff_id::bigint
      AND orphan.id != official.id;
    GET DIAGNOSTICS deleted_duplicates = ROW_COUNT;

    -- VINCULAÇÃO EM CLASSES
    UPDATE bible_class_attendees
    SET staff_id = target_staff_id::bigint, student_name = official_name
    WHERE lower(regexp_replace(student_name, '[^a-zA-Z0-9]', '', 'g')) = clean_orphan
      AND staff_id IS NULL;
    GET DIAGNOSTICS updated_classes = ROW_COUNT;
    ...
    -- VINCULAÇÃO EM ESTUDOS BÍBLICOS
    UPDATE bible_study_sessions
    SET staff_id = target_staff_id::bigint, name = official_name, participant_type = 'Colaborador', sector_id = COALESCE(sector_id, official_sector_id)
    WHERE lower(regexp_replace(name, '[^a-zA-Z0-9]', '', 'g')) = clean_orphan;
    GET DIAGNOSTICS temp_count = ROW_COUNT;
    updated_others := updated_others + temp_count;
    ...
    -- VINCULAÇÃO EM VISITAS
    UPDATE staff_visits
    SET staff_id = target_staff_id::bigint, staff_name = official_name, participant_type = 'Colaborador'
    WHERE lower(regexp_replace(staff_name, '[^a-zA-Z0-9]', '', 'g')) = clean_orphan;
    GET DIAGNOSTICS temp_count = ROW_COUNT;
    updated_others := updated_others + temp_count;
...
DROP FUNCTION IF EXISTS unify_identity_global(text, text);
