...
const HistoryCard: React.FC<HistoryCardProps> = ({ icon, color, title, subtitle, chaplainName, isLocked, isAdmin, users, onEdit, onDelete, onContinue, onTransfer, extra, middle, flag }) => {
  ...
  return (
    <div className="bg-white p-5 md:p-6 rounded-[2.5rem] border border-slate-100 flex flex-col md:row md:items-center justify-between shadow-sm hover:border-blue-200 transition-all group gap-4">
      ...
      <div className="flex items-center justify-between md:justify-end gap-3 border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-4">
        ...
        <div className="flex items-center gap-1.5 ml-auto md:ml-0">
          {onContinue && !isLocked && (
            <button 
              onClick={onContinue} 
              className="px-4 h-10 bg-blue-600 text-white rounded-xl hover:bg-blue-700 active:scale-95 transition-all flex items-center gap-2 shadow-sm"
              title="Continuar Estudo/Classe"
            >
