        ...
        {myRequests.map(req => {
          ...
          return (
            <div key={req.id} className={`min-w-[260px] max-w-[280px] p-4 rounded-2xl border flex flex-col justify-between relative group transition-all ${
              // Mantido o destaque azul original -- só acrescentado o brilho pulsante
              // (animate-today-glow, ver index.css) por cima dele, a pedido do usuário.
              isToday ? 'bg-blue-50/50 border-blue-200 ring-1 ring-blue-100 animate-today-glow' : 'bg-slate-50 border-slate-100 hover:border-blue-200'
            }`}>
