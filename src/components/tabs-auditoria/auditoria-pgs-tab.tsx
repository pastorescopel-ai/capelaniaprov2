...
interface AuditoriaPGsTabProps {
  duplicatePGs: any[];
  handleMergePGs: (sourceId: string, targetId: string) => void;
  isProcessing: boolean;
}

const AuditoriaPGsTab: React.FC<AuditoriaPGsTabProps> = ({ duplicatePGs, handleMergePGs, isProcessing }) => {
        ...
        {duplicatePGs.map((dup, idx) => (
          <div key={idx} className="bg-white rounded-[2.5rem] shadow-xl border border-slate-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-500" style={{ animationDelay: `${idx * 100}ms` }}>
            <div className="p-6 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
              ...
              <div className="bg-amber-100 text-amber-700 px-3 py-1 rounded-lg text-[9px] font-black uppercase">{dup.groups.length} Cadastros Detectados</div>
            </div>
              ...
              {selectedTargets[dup.name] && (
                <div className="pt-4 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
                  <div className="flex items-center gap-3 text-amber-600">
                    ...
                    <p className="text-[10px] font-bold uppercase leading-tight">Os dados históricos dos outros cadastros serão movidos para o ID <span className="font-black">{selectedTargets[dup.name]}</span>.</p>
                  </div>
                  <div className="flex gap-2">
                    {dup.groups.filter((g: any) => g.id !== selectedTargets[dup.name]).map((sourceGroup: any) => (
                      <button
                        key={sourceGroup.id}
...
export default AuditoriaPGsTab;
