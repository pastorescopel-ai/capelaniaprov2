...
interface AuditoriaAttendeesTabProps {
  attendeeOrphans: any[];
...
const AuditoriaAttendeesTab: React.FC<AuditoriaAttendeesTabProps> = ({
  attendeeOrphans,
  isLoadingAttendees,
  personTypeMap,
  setPersonTypeMap,
    ...
    <div className="bg-white rounded-[3rem] shadow-xl border border-slate-100 overflow-hidden">
      <div className="p-8 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
        ...
        <span className="text-[10px] font-bold text-slate-400 uppercase">
          <i className="fas fa-magic mr-1"></i> Correção em massa
        </span>
      </div>
      <div className="divide-y divide-slate-100 max-h-[65vh] overflow-y-auto custom-scrollbar pb-72">
        {isLoadingAttendees ? (
          <div className="p-20 text-center">
            <i className="fas fa-circle-notch fa-spin text-3xl text-violet-400"></i>
            <p className="mt-4 text-xs font-bold text-slate-400 uppercase">Analisando banco de dados...</p>
          </div>
              ...
              <div key={index} className="p-6 flex flex-col xl:flex-row items-center gap-6 hover:bg-slate-50 transition-colors group">
                ...
                <div className="flex-1 w-full xl:w-2/3 flex flex-col md:flex-row gap-3">
                  <div className="flex-1">
                    <Autocomplete
                      options={currentType === "Colaborador" ? officialStaffOptions : currentType === "Paciente" ? officialPatientOptions : officialProviderOptions}
...
export default AuditoriaAttendeesTab;
