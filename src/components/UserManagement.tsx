...
import { OnlineUsersPanel } from './Team/OnlineUsersPanel';
...
const UserManagement: React.FC<UserManagementProps> = ({ users, currentUser, onUpdateUsers }) => {
  ...
  const [activeTab, setActiveTab] = useState<'management' | 'online'>('management');
  ...
  return (
    <div className="space-y-12 max-w-5xl mx-auto pb-24 animate-in fade-in duration-500">
      <header className="flex justify-between items-start">
        ...
        <div className="bg-white p-1 rounded-full flex border border-slate-100 shadow-sm">
          {[
            { id: 'management', label: 'Cadastro da Equipe', icon: '👥' },
            { id: 'online', label: 'Quem está online', icon: '🟢' }
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id as any)}
              className={`px-6 py-3 rounded-full text-xs font-black uppercase flex items-center gap-2 transition-all ${
                activeTab === tab.id 
      ...
      {activeTab === 'online' ? (
        <OnlineUsersPanel currentUser={currentUser} allUsers={users} />
      ) : (
        <>
            {/* MODAL: PROCESSAMENTO */}
            {isProcessing && (
