export const COLLECTION_TO_TABLE: Record<string, string> = {
  ...
  bibleClassAdventists: 'bible_class_adventists',
  proMonthlyStats: 'pro_monthly_stats',
  proHistoryRecords: 'pro_history_records'
};

export const TABLE_SCHEMAS: Record<string, string[]> = {
  users: ['id', 'name', 'email', 'password', 'role', 'profile_pic', 'attends_haba', 'haba_days', 'created_at', 'updated_at', 'auth_id', 'staff_id', 'daily_visit_goal', 'subunit_monthly_visit_goal', 'visit_goal_period', 'last_login_at'],
  ...
  bible_class_adventists: ['id', 'class_id', 'student_name', 'staff_id', 'participant_id', 'date', 'cycle_month', 'created_at', 'updated_at', 'unit'],
...
export const NUMERIC_FIELDS = ['font_size1', 'font_size2', 'font_size3', 'report_logo_width', 'report_logo_x', 'report_logo_y', 'header_line1_x', 'header_line1_y', 'header_line2_x', 'header_line2_y', 'header_line3_x', 'header_line3_y', 'header_padding_top', 'participants_count', 'provider_id', 'group_id', 'sector_id', 'day_of_week', 'total_staff', 'total_participants', 'active_groups', 'percentage', 'goal', 'participant_id', 'terminal_count', 'clinical_count', 'daily_visit_goal', 'subunit_monthly_visit_goal'];
...
export const cleanAndConvertToSnake = (obj: any, allowedFields: string[], tableName: string): any => {
  ...
  for (const key in obj) {
    ...
    if (allowedFields.includes(snakeKey)) {
      ...
      const isTargetTable = tableName.startsWith('pro_') || tableName === 'bible_study_sessions' || tableName === 'bible_class_attendees' || tableName === 'bible_class_adventists' || tableName === 'staff_visits' || tableName === 'bible_classes';
      ...
      if (tableName !== 'visit_requests' && tableName !== 'daily_activity_reports' && !tableName.startsWith('pro_') && snakeKey !== 'staff_id' && snakeKey !== 'provider_id' && snakeKey !== 'sector_id') {
        if (isFK && val && !isValidUUID(val) && tableName !== 'bible_class_attendees' && tableName !== 'bible_class_adventists' && !String(val).match(/^\d+$/)) {
