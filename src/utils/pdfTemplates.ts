  ...
  const renderTable = (unit: Unit) => {
    ...
    return `
      <div style="margin-bottom: 40px;">
        <h4 style="margin: 0 0 10px 0; font-size: 14px; font-weight: 900; color: ${pColor}; text-transform: uppercase; border-left: 5px solid ${pColor}; padding-left: 10px;">Unidade ${unit}</h4>
        <table style="width: 100%; border-collapse: collapse; font-size: 10px; table-layout: fixed;">
          ...
          <tbody>
            ${tableData.map(s => {
              const u = unit === Unit.HAB ? s.hab : s.haba;
              return `
                <tr style="border-bottom: 1px solid #e2e8f0;">
                  <td style="padding: 10px; font-weight: 700; color: #1e293b;">${s.name}</td>
                  <td style="padding: 10px; text-align: center; font-size: 14px; font-weight: 900; color: ${pColor}; background: #f8fafc;">${u.students}</td>
    ...
    <div class="pdf-page" style="width: 210mm; height: 297mm; background: white; box-sizing: border-box; font-family: sans-serif; position: relative; overflow: hidden; display: flex; flex-direction: column;">
      ...
      <div style="padding: 0 15mm 15mm 15mm; flex: 1;">
        ...
        <div style="margin-top: 20px; display: grid; grid-template-columns: repeat(5, 1fr); gap: 10px; padding-top: 20px; border-top: 2px solid #f1f5f9;">
          ${[
            {l: 'Média de Alunos (Mensal)', v: totalStats.averageStudentsMonthly, c: '#dc2626'},
            {l: 'Estudos Bíblicos Individuais', v: totalStats.uniqueIndividualStudents, c: '#3b82f6'},
