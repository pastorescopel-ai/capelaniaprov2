...
const AdminLists: React.FC<AdminListsProps> = ({ lists, setLists, onAutoSave, proData, onSavePro }) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const res: PreviewItem[] = dataRows.map(row => {
            ...
            if(seenIds.has(finalId)) return null; 
            seenIds.add(finalId);
  ...
  const handleConfirmImport = async () => {
    ...
    setSyncState({ isOpen: true, status: 'processing', title: 'Salvando Dados', message: 'Atualizando banco numérico...' });
      ...
      <section className="bg-white p-10 rounded-[3rem] shadow-sm border border-slate-100 space-y-8">
        ...
        <div className="bg-slate-50 p-6 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-4 w-full md:w-auto">
                ...
                <div className="text-xs font-bold text-slate-400">{previewData.length > 0 ? <span className="text-blue-600">{previewData.length} registros lidos.</span> : <span>Banco Atual: {displayData.length} registros</span>}</div>
            </div>
                                        ...
                                        item.sectorStatus === 'ok' ? (
                                            <div className="flex items-center justify-between group">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[10px] font-black text-slate-700 uppercase">{getSectorName(item.sectorIdLinked!)}</span>
                                                </div>
                                                <button onClick={() => handleForceEdit(i)} className="w-6 h-6 rounded-lg bg-slate-50 text-slate-300 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-all"><i className="fas fa-pencil-alt text-[10px]"></i></button>
                                            </div>
                                        ) : (
                                            <Autocomplete options={sectorOptions} value={item.sectorNameRaw || ''} onChange={(val) => handleManualSectorChange(i, val)} placeholder="⚠️ Corrigir Setor..." required={false} className="w-full p-2 text-xs font-bold rounded-xl border-2 border-amber-300 bg-white" />
                                        )
                                    ) : (
                                        <span className="text-[10px] font-bold uppercase text-slate-500">{getSectorName(item.sectorId)}</span>
                                    )}
                                </td>
                            )}
