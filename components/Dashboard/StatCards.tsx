...
const StatCards: React.FC<{ stats: Stat[] }> = ({ stats }) => {
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
      {stats.map((stat) => (
        <div key={stat.label} className="bg-white p-5 rounded-3xl flex flex-col items-center text-center group hover:-translate-y-1 transition-all cursor-default border border-slate-200 shadow-sm">
